    ...
    <div>
        <div class="flex justify-end">
            <form wire:submit.prevent="createCategory">
                <x-text-input wire:model="nombre" type="text" placeholder="Nueva categoría" />
        ...
        @foreach ($categories as $category)
            <div class="mt-8">
                <div class="bg-white dark:bg-gray-900 rounded-t-lg p-3 text-left">
                    <p class="text-white font-bold text-lg">Nombre: {{$category->nombre}}</p>
                    <p class="text-white font-bold text-sm">id: {{$category->id}}</p>
                </div>
                <div class="flex w-full">
                    <button
                        wire:click="$emit('showAlertDelete',{{$category->id}})"
...
<script>
    Livewire.on('showAlertDelete', categoryId => {
        Swal.fire({
