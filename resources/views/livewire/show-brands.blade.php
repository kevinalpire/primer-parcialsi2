    ...
    <div>
        <div class="flex justify-end">
            <form wire:submit.prevent="createBrand">
                <x-text-input wire:model="nombre" type="text" placeholder="Nueva marca" />
        ...
        @foreach ($brands as $brand)
            <div class="mt-8">
                <div class="bg-white dark:bg-gray-900 rounded-t-lg p-3 text-left">
                    <p class="text-white font-bold text-lg">Nombre: {{$brand->nombre}}</p>
                    <p class="text-white font-bold text-sm">id: {{$brand->id}}</p>
                </div>
                <div class="flex w-full">
                    <button
                        wire:click="$emit('showAlertDelete',{{$brand->id}})"
...
<script>
    Livewire.on('showAlertDelete', brandId => {
        Swal.fire({
